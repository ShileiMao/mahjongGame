...
exports.createRoom = function(creator,roomConf,gems,ip,port,callback){
		...
		*/
		...
		console.log("room conf: " + JSON.stringify(roomConf));

	if(
		roomConf.type == null 
		|| roomConf.difen == null  // 底分（每局起始分数）int
		|| roomConf.zimo == null // 自摸 int
		|| roomConf.jiangdui == null // 
		|| roomConf.huansanzhang == null // 换三张 （血流成河打法）
		|| roomConf.zuidafanshu == null // 最大蕃数
	...
	// if(roomConf.difen < 0 || roomConf.difen >  DI_FEN.length){
		if(roomConf.difen < 0 || roomConf.difen >  Math.max(...DI_FEN)){
		console.log("--2")
		callback(1,null);
		return;
	}
	...
	if(roomConf.zuidafanshu < 0 || roomConf.zuidafanshu > Math.max(...MAX_FAN)){
		console.log("--4")
		callback(1,null);
		return;
	}

	if(roomConf.jushuxuanze < 0 || roomConf.jushuxuanze > Math.max(...JU_SHU)){
		console.log("--5")
		callback(1,null);
		return;
	}
...
exports.enterRoomFake = function(roomId,userId,seatId) {
	var roomInfo = {
		uuid: "uuid123" + roomId,
		id: roomId,
		numOfGames: 10,
		...
		nextButton: 0,
		seats: new Array(4),
		conf:JSON.parse(`{"type":"xzdd","baseScore":1,"zimo":0,"jiangdui":false,"hsz":false,"dianganghua":0,"menqing":false,"tiandihu":false,"maxFan":4,"maxGames":4,"creator":9}`)
	};
