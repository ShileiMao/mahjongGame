...
export class DingQue extends Component {
  ...
  initEventHandlers() {
    ...
    this.node.on("game_dingque_notify", function (data) {
      console.log("game_dingque_notify: " + data);
  ...
  setInteractable(value: boolean) {
    this.queYiMen.getChildByName("tong").getComponent(Button).interactable =
      value;
    this.queYiMen.getChildByName("tiao").getComponent(Button).interactable =
      value;
    this.queYiMen.getChildByName("wan").getComponent(Button).interactable =
      value;
  }
}
