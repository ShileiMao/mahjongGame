           ...
           for(var j = 0; j < userData.huinfo.length; ++j){
               ...
               if(info.ishupai){
                   if(hi < hupaiRoot.children.length){
                       ...
                       hupaiView.getComponent(Sprite).spriteFrame = AppGlobal.vv().mahjongmgr.getSpriteFrameByMJID("B_",info.pai);
                       hi++;   
                   }
               }
               var str = ""
    ...
    initPengAndGangs (seatView: any, index: any, mjid: any, flag: any) {
        ...
        for(var s = 0; s < sprites.length; ++s){
           ...
           if(sprite.node.name == "gang"){
               ...
               if(flag == "angang"){
                   sprite.spriteFrame = AppGlobal.vv().mahjongmgr.getEmptySpriteFrame("myself");
                   sprite.node.scaleX = 1.4;
                   sprite.node.scaleY = 1.4;                        
               }   
               else{
                   sprite.spriteFrame = AppGlobal.vv().mahjongmgr.getSpriteFrameByMJID("B_",mjid);    
               }
           }
           else{ 
               sprite.spriteFrame = AppGlobal.vv().mahjongmgr.getSpriteFrameByMJID("B_",mjid);
