import { _decorator, Button, Component, Label, Node, Sprite, Animation } from 'cc';
...
export class Seat extends Component {
    ...
    onLoad () {
        ...
        if(this._sprIcon && this._sprIcon.getComponent(Button)){
           ...
           AppGlobal.vv().utils.addClickEvent(this._sprIcon, this.node, "Seat","onIconClicked");    
        }
      ...
      console.log("on icon clicked!!")
        ...
        if(this._userId != null && this._userId > 0){
          var seat = AppGlobal.vv().gameNetMgr.getSeatByID(this._userId);
           ...
           AppGlobal.vv().userinfoShow.show(seat.name,seat.userid,iconSprite,sex,seat.ip);         
        }
    }
    ...
    refresh () {
        ...
        if(this._ready){
           this._ready.active = this._isReady && (AppGlobal.vv().gameNetMgr.numOfGames > 0); 
        }
        if(this._zhuang){
    ...
    chat (content: any) {
        ...
        this._chatBubble.getChildByName("New Label").getComponent(Label).string = content;
