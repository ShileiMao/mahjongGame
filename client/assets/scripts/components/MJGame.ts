...
export class MJGame extends Component {
  ...
  initView() {
    ...
    this._gamecount.string =
      "" +
      AppGlobal.vv().gameNetMgr.numOfGames +
      "/" +
      AppGlobal.vv().gameNetMgr.maxNumOfGames +
      "局";
    ...
    for (var i = 0; i < myholds.children.length; ++i) {
      ...
      this.initDragStuffs(myholds.children[i]);
    }
    var realwidth = view.getVisibleSize().width;
    AppGlobal.vv().utils.setScale(myholds, {
      x: realwidth / 1280,
      y: realwidth / 1280,
    });
    ...
    for (var i = 0; i < sides.length; ++i) {
      ...
      this._playEfxs.push(
        sideChild.getChildByName("play_efx").getComponent(Animation)
      );
      this._chupaiSprite.push(
        sideChild.getChildByName("ChuPai").children[0].getComponent(Sprite)
      );
  ...
  initDragStuffs(node: Node) {
    node.on(
      ...
      function (event) {
        ...
        const interactable = node.getComponent(Button).interactable;
        if (!interactable) {
          return;
        }
    ...
    node.on(
      ...
      function (event) {
        ...
        node.getComponent(UIOpacity).opacity = 150;
  ...
  initEventHandlers() {
    ...
    this.node.on("game_chupai", function (data) {
      ...
      if (
        !AppGlobal.vv().replayMgr.isReplay() &&
        data.turn != AppGlobal.vv().gameNetMgr.seatIndex
      ) {
    ...
    this.node.on("hupai", function (data) {
      ...
      if (
        AppGlobal.vv().replayMgr.isReplay() == true &&
        AppGlobal.vv().gameNetMgr.conf.type != "xlch"
      ) {
        var opt = self._opts[localIndex];
        opt.node.active = true;
        opt.sprite.spriteFrame = AppGlobal.vv().mahjongmgr.getSpriteFrameByMJID(
          "M_",
          data.hupai
        );
      }
    ...
    if (AppGlobal.vv().gameNetMgr.conf.type == "xlch") {
      ...
      for (var i = 0; i < hupailist.children.length; ++i) {
        ...
        if (hupainode.active == false) {
          ...
          hupainode.getComponent(Sprite).spriteFrame =
            AppGlobal.vv().mahjongmgr.getSpriteFrameByMJID(pre, pai);
  ...
  onGameBeign() {
    ...
    if (
      AppGlobal.vv().gameNetMgr.gamestate == "" &&
      AppGlobal.vv().replayMgr.isReplay() == false
    ) {
      return;
    }
  ...
  initMopai(seatIndex: any, pai: any) {
    ...
    if (pai == null) {
      ...
      var sprite = nc.getComponent(Sprite);
      sprite.spriteFrame =
        AppGlobal.vv().mahjongmgr.getHoldsEmptySpriteFrame(side);
    }
  }
  ...
  checkQueYiMen() {
    console.log("on game begin !!!!!!!!");
    if (
      AppGlobal.vv().gameNetMgr.conf == null ||
      AppGlobal.vv().gameNetMgr.conf.type != "xlch" ||
      !AppGlobal.vv().gameNetMgr.getSelfData().hued
    ) {
      var dingque = AppGlobal.vv().gameNetMgr.dingque;
      var hasQue = false;
      if (
        AppGlobal.vv().gameNetMgr.seatIndex == AppGlobal.vv().gameNetMgr.turn
      ) {
        for (var i = 0; i < this._myMJArr.length; ++i) {
          var sprite = this._myMJArr[i];
          if ((sprite.node as any).mjId != null) {
            var type = AppGlobal.vv().mahjongmgr.getMahjongType(
              (sprite.node as any).mjId
            );
            if (type == dingque) {
              hasQue = true;
              break;
